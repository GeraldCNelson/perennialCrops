...
getChillSpatial <- function(years, lat, JDay, tmin, tmax, template, hem, writeToDisk=FALSE,...) {
  ...
  print("Starting getChillSpatial"); print(Sys.time())
  systimestart <- Sys.time()
  # add days to JDay to account for omissions on either side
  JDay <- c(JDay[1]-1, JDay, JDay[length(JDay)]+1)
  ...
  if(class(tmin)=='list') {
    ...
    for(i in seq_along(yearRange)) {
      ...
      # brick_empty <- brick(nrows=180, ncols=720, xmn=-180, xmx=180, ymn=0, ymx=90, nl=length(period), crs = "+proj=longlat +datum=WGS84 +no_defs" )
      
    #  ext_hem <- ext_NH
      ext_hem <- get(paste0("ext_", hem))
...
getChillWorld <- function(scenario, model, year_range) {
  ...
  # dat.dir <- paste0('climdata/', scenario, '/')
  ...
  t <- proc.time()
  chill_portions.north <- getChillSpatial(years=year_range.north, lat, JDay.north, tmin=tmin.north, tmax=tmax.north, template=template.ras, hem)
  t1 <- t-proc.time()
  t1/60
  
  newDir <- paste0('data/chill_portions/', scenario, '/', model, '/', year_range[10])
  ...
  writeRaster(chill_portions.north.raster,
              paste0('data/chill_portions/',
                     scenario, '/', model, '/', scenario, '_', model, '_', year_range[10], '_', 'chill_portions_north.tif'), overwrite=TRUE)
  
  rm(tmin.north, tmax.north)
  gc()
  print("Done with NH"); print(Sys.time())
  systimeendNH <- Sys.time()
  #### southern hemisphere #### ------
  hem <- "SH"
  if(all(year_range==1991:2010)) {
    tmin.in <- rast(tmin.files[1])
 ...
 # tmax.south <- crop(tmax.in, ext(c(-180,180,-60,0)))
  tmin.south <- crop(tmin.in, ext_SH) # -60 no antarctica
  ...
  # process daily temperatures
  t <- proc.time()
  chill_portions.south <- getChillSpatial(years=year_range, lat, JDay.south, tmin=tmin.south, tmax=tmax.south, template=template.ras, hem)
  t1 <- t-proc.time()
  t1/60

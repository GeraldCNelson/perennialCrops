...
#speciesNames <- unique(cropVals$cropName)
...
f_readRast_count <- function(k, l, modelChoice, climateVarChoice, threshold, layersToKeep, probVal, hem) {
...
f_extremeCold <- function(k, l, speciesName, hem, modelChoices_lower, cropVals) {
  gddFilesCompleted <- list.files(path,  full.names = TRUE)
  ...
  fileName_out <- paste0(path, "extremeCold_cutoff_", speciesName, "_", k, "_", hem, "_", yearSpan, ".tif")
...
f_gddSums <- function(k, l, speciesName, hem, runsParms, modelChoice) {
  ...
  if (hem == "SH")  {
    startDate <-  paste0(l, "-07-01"); endDate <- paste0(l + yearRange-1, "-06-30") # in southern hemisphere search July 1 to June 30 of the next year. 
  ...
  if (fileName_gddSums_out %in% gddSumFilesCompleted) {
    ...
    for (yearNumber in 1:nlyr(startDay)) {
      ...
      if (hem == "SH")  {
        startDate <-  paste0(startYear, "-07-01"); endDate <- paste0(startYear + 1, "-06-30")
...
f_gddSum_mean <- function (k, l, speciesNames, modelChoices, hemispheres) {
  yearSpan <- paste0(l, "_", l + yearRange)
...
f_frostDamage <- function(k, l, speciesName, hem, suitabilityLevel, cropVals) {
  ...
  layersToKeep <- spLyrs
  probVal <- 0.90
  system.time(x <- lapply(modelChoices_lower, f_readRast_count, k, l, climateVarChoice, threshold, layersToKeep, probVal, hem))
  r <- rast(x)
  r[r <= frDays[2]] <- 1
  r[r > frDays[2]] <- 0
  #       fr <- f_range(r, frDays)
...
f_heatDamage <- function(k, l, speciesName, hem, suitabilityLevel, cropVals) {
  yearSpan <- paste0(l, "_", l + yearRange)
  ...
  threshold <- cropVals[cropName == speciesName, summer_heat_threshold]
  layersToKeep <- hdLyrs
  probVal <- 0.90
  system.time(x <- lapply(modelChoices_lower, f_readRast_count, k, l, climateVarChoice, threshold, layersToKeep, probVal, hem))
  r <- rast(x)
...
f_suitableLocsGraphics <- function(k, l, speciesName, suitabilityLevel) {
  ...
  r_df <- project(r, crsRob) |> as.data.frame(xy = TRUE) |> round(0)
...
f_suitableLocsGraphics_clean_old <- function(speciesName) {
  ...
  # convert to vector for a border and project
  harvestArea_earlyCent_p_sf_rob <- as.polygons(harvestArea_earlyCent) |> st_as_sf() |> st_transform(crsRob)
  
  harvestArea_df <- project(harvestArea_earlyCent, crsRob) |> as.data.frame(xy = TRUE) |> round(0)
  names(harvestArea_df) <-   c("x", "y", "value_harvest")
  harvestArea_df[harvestArea_df == 0] <- NA

...
{  
  ...
  return(list(Sunrise = Sunrise, Sunset = Sunset, Daylength = Daylength, JDay=rep(JDay, each=length(Sunrise))))
}
...
getCP <- function (tmin=tmin, tmax=tmax, dates, Day_times=daytimes, keep_sunrise_sunset = FALSE, template=NULL, datClass, ...) 
{
  ...
  tmax <- as.matrix(tmax)
  tmin <- as.matrix(tmin)
  # print("end matrix conversion"); print(Sys.time())
...
getChillSpatial <- function(years, lat, JDay, tmin, tmax, template, hem, writeToDisk=FALSE,...) {
  ...
  if(class(tmin)=='list') {
    ...
    #    browser()
   #     temp <- names(tmin)
        dates <- as.Date(gsub("X", "", names(tmin)))
   #     dates <- as.Date(1:nlyr(tmin), origin=as.Date(paste0(years[1], "-01-01"))-1)
      }
    }
    ...
    for (i in seq_along(yearRange)) {
      ...
      browser()
      system.time( m_tmin <- as.matrix(tmin))
      system.time(m_tmax <- as.matrix(tmax))
      tmin.tmp <- tmax.tmp <- brick_empty
      system.time(tmin.tmp[] <- m_tmin)
      system.time(tmax.tmp[] <- m_tmax)
...
getChillWorld <- function(scenario, model, year_range) {
  ...
  tmax.files <- grep("tasmax", dat.files, value=TRUE)
   print("starting on cropping")
  if(all(year_range==1991:2010)) {
    system.time(tmin.north <- rast(tmin.files[1], win = ext_NH, lyrs = JDay.north))
    system.time(tmax.north <- rast(tmax.files[1], win = ext_NH, lyrs = JDay.north)) 
    system.time(tmin.south <- rast(tmin.files[1], win = ext_NH, lyrs = JDay.south))
    system.time(tmax.south <- rast(tmax.files[1], win = ext_NH, lyrs = JDay.south))
    # tmin.north <- crop(tmin.in, ext_NH)
    # tmax.north <- crop(tmax.in, ext_NH)
    ...
    system.time(tmin.north <- rast(tmin.files[2], win = ext_NH, lyrs = JDay.north))
    system.time(tmax.north <- rast(tmax.files[2], win = ext_NH, lyrs = JDay.north)) 
    system.time(tmin.south <- rast(tmin.files[2], win = ext_NH, lyrs = JDay.south))
    system.time(tmax.south <- rast(tmax.files[2], win = ext_NH, lyrs = JDay.south))
    # tmin.north <- crop(tmin.in, ext_NH)
    # tmax.north <- crop(tmax.in, ext_NH)
  } else {
    stop("Unsupported year range specified.")
  }
  print("ending cropping")
  ...
  print("check 1.25"); print(Sys.time())
  
  #  tmin.north <- crop(tmin.in, ext(c(-180,180,0,90)))
  #  print("check 1.5"); print(Sys.time())
  ...
  # this next line of code doesn't work because tmin.north is already cropped. Lat north doesn't change by day so all the really needs it to create a lat raster for as many days as needed
 # lat_north <- tmin.north[[c(JDay.north[1]-1, JDay.north, JDay.north[length(JDay.north)]+1)]]
  # use lat generating functioninstead
  jDayHem <- JDay.globe <- 1:365
  year <- year_range[1]
  lat_north <- create_lats(hem = "NH", jDayHem, year)[1]
  ...
  template.ras <- rast(, ncols = 720, nrows = 180, nlyrs = 1, crs = "EPSG:4326", ext = ext_NH)
  # process daily temperatures
  year_range.north <- year_range[1:(length(year_range)-1)]
  t <- proc.time()
  chill_portions.north <- getChillSpatial(years=year_range.north, lat, JDay.north, tmin=tmin.north, tmax=tmax.north, template=template.ras, hem <- "NH") # a list of 19 raster layers
  t1 <- t-proc.time()
  t1/60
  
  # d <- paste0('data/chill_portions/', scenario, '/', model, '/', year_range[10], "/")
  d <- paste0('data/chill_portions/')
  ...
  outF_NH <- paste0(d, scenario, "_", model, "_chill_portions_north.tif")
  writeRaster(chill_portions.north.raster, outF_NH, overwrite=TRUE)
  print(paste0("outF_NH: ", outF_NH))
  print(chill_portions.north.raster)
  ...
  if(all(year_range==1991:2010)) {
    ...
    tmin.south <- crop(tmin.in, ext_SH)
    tmax.south <- crop(tmax.in, ext_SH)
  } else if (all(year_range==2041:2060)) {
    tmin.in <- rast(tmin.files[1]) 
    tmax.in <- rast(tmax.files[1]) 
    tmin.south <- crop(tmin.in, ext_SH)
  ...
  # 
  # tmin.north <- rast(tmin.files[1], win = ext(c(-180, 180, 0, 90))) 
  # tmax.north <- rast(tmax.files[1], win = ext(c(-180, 180, 0, 90)))
  # 
  ...
  # produce spatial layer of latitudes and use to calculate day lengths
  lat_south <- tmin.south[[c(JDay.south[1]-1, JDay.south, JDay.south[length(JDay.south)]+1)]]
 ...
 # template.ras <- tmin.south[[1]]#[[1]]
  template.ras <- rast(, ncols = 720, nrows = 180, nlyrs = 1, crs = "EPSG:4326", ext = ext_SH)
  
  # process daily temperatures
  t <- proc.time()
  chill_portions.south <- getChillSpatial(years=year_range, lat, JDay.south, tmin=tmin.south, tmax=tmax.south, template=template.ras, hem = "SH")
  t1 <- t-proc.time()
  t1/60
  ...
  #  outF_SH <- paste0(d, '_chill_portions.south.raster.tif')
  outF_SH <- paste0(d, scenario, "_", model, "_chill_portions_south.tif")
  writeRaster(chill_portions.south.raster, outF_SH, overwrite=TRUE)
  print(paste0("outF_SH: ", outF_SH))
  outF_SH

...
# commented out because now in GCBFig2Tab8.R
# # graphing ------
...
# 
# speciesNames <- speciesNames[!speciesNames == "winegrape"]
...
#   
#   # for olive there is a thin strip in the north of Europ where the lo value is less than the regular value. I'm just going to set that to zero
#   if (i %in% c("olive", "winegrape")) lo_benefits[lo_benefits < 0] <- 0
#   
...
#   harvestArea_rob_df <- as.data.frame(harvestArea_earlyCent_rob, xy = TRUE)
#   names(harvestArea_rob_df) <-   c("x", "y", "value_harvest")
#   harvestArea_rob_df <- round(harvestArea_rob_df, 0)
#   harvestArea_rob_df[harvestArea_rob_df == 0] <- NA
#   # browser()
#   fileName_out <- paste0(lofOfGraphicsFiles, "perennials/delta_suitability_", i, ".png")
#   
#   titleText <- paste0("   Suitability changes for ", i, ".")
#   legendTitle <- ""
...
#   if (i == speciesNames[speciesCount])  legendSwitch <- TRUE
#   
...
#   g <- ggplot() +
#     geom_tile(data = dplyr::filter(r_combined_df,  !is.na(value)), aes(x, y, fill = value), show.legend = legendSwitch, stat = "identity", position = "identity") +
#     scale_fill_manual(labels = c("Early century suitability lost", "Early century suitability retained", "End century, new suitable areas", "End century, suitable areas\n with low chill portions"), values = colList, na.value = 'white') +
#     
#     #        scale_discrete_manual(labels = c("Suitability lost", "Suitability retained", "New suitable areas", "New areas, end century\n with low chill portions"), values = c("red", "yellow", "mediumseagreen", "blue"), na.value = 'white') +
...
#     
#     #      early century harvest area, commented out 4/18/2022
#     # geom_tile(data = dplyr::filter(harvestArea_df, !is.na(value_harvest)), 
#     #           aes(x = x, y = y), fill = "grey24", alpha = .2, show.legend = legendSwitch) # +
...
#     scale_x_continuous(sec.axis = sec_axis(~ . , breaks = NULL, labels = NULL)) # name = "",
#   
#   print(g)
...
# plot(suit_combined_endCent_heat, main = paste0("crop ", i ,"Chill portions, heat stress and combined suitability,  \nperiod ", yearSpan_end, ", scenario ", k))
# plot(suit_all_main_end, main = paste0("crop ", i ,"Combined suitability, all, \nperiod ", yearSpan_end, ", scenario ", k))
# plot(suit_all_lo_end, main = paste0("crop ", i , "Combined suitability, all, lo chill, \nperiod ", yearSpan_end, ", scenario ", k))
...
#   #  r_combined_df[r_combined_df == 0] <- NA
#   r_combined_df$value = as.factor(r_combined_df$value)
#   titleText <- paste0("Suitability types for ", i, " end 21st century, climate scenario ", scenarioName)
#   
#   g <- ggplot() +
...
#     geom_tile(data = dplyr::filter(harvestArea_rob_df, !is.na(value_harvest)), 
...
# commented out because it is now in GCBFig2Tab8.R -----
# library(flextable)
# library(officer)
# areaTable <- areaDat
...
#   what = c("Species", #1
#            "Early Century Suitability (sq. km)", "Early Century Suitability (sq. km)", "End Century Suitability (sq. km)", #3
#            "End Century Suitability (sq. km)", "End Century Suitability (sq. km)", "End Century Suitability (1000 sq. km)", "End Century Suitability (1000 sq. km)"), #4
#   measure = c("Species", #1
...
# areaTable_flex <- colformat_num(areaTable_flex,i = 2:7, j = 3:8, na_str = "N/A")
# areaTable_flex <- width(areaTable_flex, j = 8, width = 1.5)
# areaTable_flex <- align(areaTable_flex, align = "right", part = "header")

library(terra)
e <- ext(-180, 180, -60, 90)
terraOptions(memfrac = .9, tempdir = "temp", verbose = TRUE) # for windows, 0.6 is a safer choice for memfrac
JDay.SH <- (92-2):(306)
JDay.NH <- JDay.SH + 180
JDay.globe <- 1:365
path <- paste0('data-raw/climdata')

# test data
year <- 1991
hem <- "globe"; jDayHem <- JDay.globe# test data
year_range <- 1991:2010
model <- tolower("GFDL-ESM4")

#hem choices are globe, NH, SH
getPhotoperiod <- function(hem, jDayHem, year) { # generate spatraster with latitude values and number of layers between j days
  ...
  # get photoperiod for each day of JDay period and return a raster dl with nlyr=length(jDayHem)
  photoperiodSpatial <- function(lat) {
    dl <- list()
    for(i in 1:nlyr(lat)) {
      dl[[i]] <- app(lat[[i]], function(x) meteor::photoperiod(x, doy=i))
    }
    return(rast(dl))
  }
  dl <- photoperiodSpatial(lat)
...
for (i in 1:length(yrDayCt)) {
  ...
  temp <- getPhotoperiod(hem, jDayHem, year)
  dl <- temp[[2]]
  sunrise <- temp[[3]]
  sunset <- temp[[4]]
...
# get CP's for one year - put this in an apply function to get CP's for all years in the hourly_temps list
chill_portions_1990 <- app(t_r, CP::chill_func, cores=4)
